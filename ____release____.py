...
print '------------------------Program--------------------------'
print 'Create     : by SYL (Tony shan)'
print 'Date       : '+time.asctime()
print 'On process : '+os.getcwdu();
print '------------------------Start----------------------------'
...
for f in listdir(root):
    if os.path.isfile(os.path.join(root,f)):
        fname = os.path.splitext(f);
        name=fname[0]+fname[1];
        if  name=='____Release____.zip':
        	print '>>> Attention: have  history virsion !!!'
        	raw_input('Press enter key to delate: ')
        	os.remove( name )



zip_f=zipfile.ZipFile('____Release____.zip','w',zipfile.ZIP_DEFLATED)
for f in listdir(root):
    if 	os.path.isfile(os.path.join(root,f)):
        fname = os.path.splitext(f);
        if  is_ptcfile( fname[0] ) == True:	
...
zip_f=zipfile.ZipFile('____Release____.zip','r')
if  len(zip_f.namelist() )==0:
    zip_f.close()
    print '>>> Attention: no file ziped!!!'
    os.remove( '____Release____.zip' )
